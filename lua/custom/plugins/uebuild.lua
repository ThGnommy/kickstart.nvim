...
local function highlight_errors(buf)
  ...
  for i, line in ipairs(lines) do
    if line:lower():find 'error:' then
      vim.api.nvim_buf_set_extmark(buf, ns, i - 1, 0, { end_col = #line, hl_group = 'ErrorMsg' })
    elseif line:lower():find 'warning:' then
      vim.api.nvim_buf_set_extmark(buf, ns, i - 1, 0, { end_col = #line, hl_group = 'WarningMsg' })
    end
  end
end

local current_job = { id = nil, finished = false, buf = nil, win = nil }
    ...
    config = function()
      ...
      vim.api.nvim_create_user_command('UEBuildStop', function()
        if not current_job.finished and current_job.id then
          ...
          vim.notify('Build process not found', vim.log.levels.INFO)
        end
      end, {})
    end,
  },
